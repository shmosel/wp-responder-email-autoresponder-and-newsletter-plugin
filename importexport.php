...
{
	return "admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/importexport.php";
}
...
function step1()
{
	...
	$output = '<form action="admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/importexport.php&action=wizard&step=1" method="post">';
...
function wizard()
{
	...
	switch ($_GET['step'])
	{
		...
		if (isset($_POST['firststep_wpnonce']) && check_admin_referer('firststep_wpnonce','firststep_wpnonce'))
		{
			...
			if ($moveToStep2)
			{
				redirect("admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/importexport.php&action=wizard&step=2");	
			}
		}
		else
		{
		...
		if (isset($_POST['secondstep_wpnonce']) && check_admin_referer("importwizard_wpnonce",'secondstep_wpnonce'))
		{
			...
			if ($moveToStep3)
			{
				redirect("admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/importexport.php&action=wizard&step=3");	
			}
			else
				step2();
		}
		...
		if (isset($_POST['thirdstep_wpnonce']) && check_admin_referer("importwizard_wpnonce",'thirdstep_wpnonce'))
		{
			...
			if ($moveToStep4)
			{
				redirect("admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/importexport.php&action=wizard&step=4");	
			}
			else
			{
				step3($error);
