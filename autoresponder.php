...
function _wpr_autoresponder_list()
{
	 ...
	 if (count($autoresponders))
	 {
		foreach ($autoresponders as $responder)
		{
			...
			<tr>
			   ...
			   <td><input type="button" value="Manage Messages" class="button" onclick="window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/autoresponder.php&action=manage&aid=<?php echo $responder->id ?>';" /><input type="button" value="Delete" onclick="window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/autoresponder.php&action=delete&aid=<?php echo $responder->id ?>';" class="button" />
			   </tr>
			<?php
		}
	 }
    ...
    <input type="button" value="Create New" class="button-primary" onclick="window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/autoresponder.php&action=create';" />
    <?php
}
	...
	if (isset($_POST['name']))
	{
		...
		if ($name)
		{
			...
			else
			{
				...
				?>
                <script>window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/autoresponder.php';</script>
                <?php
			}
		}
		else
...
<table>
  ...
  <tr>
   <td colspan="2"><input type="submit" value="Create" class="button" /><input type="button" value="Cancel" onclick="window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/autoresponder.php'" class="button" /></td></tr>
</table>
</form>
<?php
}
...
function _wpr_autoresponder_back2home($delay=0)
{
	?>
    <script> 
	window.setTimeout("window.location ='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/autoresponder.php'",<?php echo $delay ?>*1000);</script>
    <?php
}
?>
