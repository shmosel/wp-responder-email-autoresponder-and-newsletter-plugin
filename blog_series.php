...
function _wpr_blog_series_delete()
{
	...
	if ($_GET['confirm'] == "true")
	{
		...
		$wpdb->query($query);
		?>
        <script>window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/blogseries.php';</script><?php
	}
	else
	{
		?>
...
<br />
        <a href="<?php echo $_SERVER['REQUEST_URI'] ?>&confirm=true" class="button">Yes</a>  <a href="admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/blogseries.php" class="button">Cancel</a>
        <?php
	}
}
  ...
  <table class="widefat">
    ...
    <tr>
      ...
      <td><?php $taxonomy = get_category($series->catid);
			 echo $taxonomy->name ;
			 ?></td>
      <td><input type="button" value="Delete" onclick="window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/blogseries.php&action=delete&bid=<?php echo $series->id ?>'" class="button" /></td>
    </tr>
    <?php
	  }
	  ?>
...
<input type="button" value="Create" onclick="window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/blogseries.php&action=create';" style="margin: 10px;clear:both;" class="button" />
...
function _wpr_blog_series_create()
{
	...
	if (isset($_POST['name']))
	{
		...
		if ($name)
		{
			...
			$wpdb->query($query);
			?>
<script>window.location='admin.phppage=wp-responder-email-autoresponder-and-newsletter-plugin/blogseries.php'</script>
<?php
exit;
		}
		else
