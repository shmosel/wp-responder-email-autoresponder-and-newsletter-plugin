...
function getNewsletterOptions($post_id=0)
{
    ...
    if (count($results))
        {
?>
<span style="font-size: 12px; line-height: 25px;">Each of the tabs below allow you to customize the email that goes out to the blog subscribers, blog category subscribers and post series subscribers of that newsletter.
</span>
...
<?php
            ...
            setting blog post email settings.<?php
        }
   ?>
</div>
   <?php
...
<p><input type="checkbox" <?php if ($arguments['disable']) { echo 'checked="checked"'; } ?> autocomplete="off" onchange="toggleStatus(<?php echo $nid ?>,this.checked);" id="toggleForm-<?php echo $nid; ?>" name="skipnewsletter-<?php echo $nid; ?>" value="1" id="skipnewsletter-<?php echo $nid; ?>"> Don't deliver this blog post to subscribers of this newsletter at all.</p>
<p><input type="checkbox" <?php if ($arguments['nopostseries']==1) { echo 'checked="checked"'; } ?> name="nopostseries-<?php echo $nid ?>" autocomplete="off" id="nopostseries"> <label for="nopostseries"> Don't use these settings while delivering email to post series subscribers.</label></p>
<div id="editorformitems-<?php echo $nid; ?>">
<p><input autocomplete="off" type="checkbox"  <?php if ($arguments['skipactivesubscribers']) { echo 'checked="checked"'; } ?> name="skipfollowup-<?php echo $nid ?>" value="1" id="skipfollowup-<?php echo $nid ?>"> Don't deliver this blog post to subscribers who are receiving a follow up sequence associated with this newsletter.</p>
<p><input type="checkbox"  <?php if ($arguments['nocustomization']) { echo 'checked="checked"'; } ?> autocomplete="off" onchange="toggleCustomization(<?php echo $nid ?>, this.checked);" name="disablecustomization-<?php echo $nid ?>" value="1" id="disablecustomization-<?php echo $nid ?>"> Disable all customization (use default layout for the email)</p>
<div id="customizationsform-<?php echo $nid ?>">
<div id="form-<?php echo $nid ?>">
    <p style="font-size: 17px; font-weight: bold">Subject: <input type="text" name="subject-<?php echo $nid ?>" value="<?php echo $arguments['subject'] ?>" size="50">
...
<p style="font-size: 17px; font-weight: bold">HTML Body:</p><?php CreateNewTemplateSwitcherButton("listOfEditors[".$nid."]","htmlbody-".$nid,$nid); ?>
<p><input type="checkbox" <?php if ($arguments['htmlenable']) { echo 'checked="checked"'; } ?> onchange="toggleHtmlBody(<?php echo $nid ?>,this.checked)" name="htmlenable-<?php echo $nid ?>" id="htmlenable-<?php echo $nid ?>" value="1">Enable HTML Body</p>
<div id="htmlformitems-<?php echo $nid ?>">
<p><input type="checkbox" name="attachimages-<?php echo $nid ?>"  <?php if ($arguments['attachimages']) { echo 'checked="checked"'; } ?>  value="1"> Images in the email are attached with the email instead of being given a URL in the source code</p>
<div id="editor-<?php echo $nid ?>">
    
    <textarea id="htmlbody-<?php echo $nid ?>" name="htmlbody-<?php echo $nid ?>" rows="20" cols="80">
    <?php echo ($arguments['htmlenable'])?$arguments['htmlbody']:""; ?></textarea></div>

<input type="button" onclick="createEditor(<?php echo $nid ?>)" value="Enable WYSIWYG" name="enable"/><input type="button" value="Disable WYSIWYG" onclick="removeEditor(<?php echo $nid; ?>)" name="enable"/>
</div></div>
...
Use the following codes in the Text or HTML body to replace it with the appropriate
text.<p></p>
<ol>
    <li>Post related shortcodes:<br><ol>
    ...
    <li>[!unsubscribe!] - Unsubscribe link (you can place this even in the HTML source)</li>
