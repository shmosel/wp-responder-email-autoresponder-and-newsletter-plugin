...
function wpr_validateEmail($email)
...

{
	...
	$name = _wpr_sanitize($_POST['name']);

	$email = strtolower(_wpr_sanitize($_POST['email']));

	$followup = _wpr_sanitize($_POST['followup']);

	$newsletter = (int) _wpr_sanitize($_POST['newsletter']);

	$bsubscription = _wpr_sanitize($_POST['blogsubscription']);

	$responder = (int) _wpr_sanitize($_POST['responder']);

	$bcategory = (int) _wpr_sanitize($_POST['cat']);

	$return_url = _wpr_sanitize($_POST['return_url']);
	...
	if (!wpr_validateEmail($email))   //the expression is just for now.
